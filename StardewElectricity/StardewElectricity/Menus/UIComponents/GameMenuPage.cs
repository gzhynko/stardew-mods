...
namespace StardewElectricity.Menus.UIComponents;

/// <summary>
/// Generic menu implementation based on a root <see cref="IView"/>.
/// </summary>
public abstract class GameMenuPage<T> : IClickableMenu, IDisposable where T : IView
{
    /// <summary>
    ...
    public GameMenuPage(int xPos, int yPos, int? fixedWidth = null, int? fixedHeight = null, Edges? gutter = null)
    {
        ...
        width = fixedWidth ?? (int)MathF.Round(view.OuterSize.X);
        height = fixedHeight ?? (int)MathF.Round(view.OuterSize.Y);
    ...
    public override void receiveKeyPress(Keys key)
    {
        ...
        if (key == Keys.Escape
            ...
            || (Game1.options.gamepadControls && StardewValley.Utility.getHeldButtons(Game1.input.GetGamePadState()).Count > 0))
        {
