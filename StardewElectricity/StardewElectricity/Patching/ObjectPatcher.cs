...
public class ObjectPatcher : BasePatcher
{
    public override void Apply(Harmony harmony, IMonitor modMonitor)
    {
        harmony.Patch(
            ...
            prefix: new HarmonyMethod(GetType(), nameof(MinutesElapsed_Prefix))
        );
        harmony.Patch(
            AccessTools.Method(typeof(Object), nameof(Object.ShouldTimePassForMachine)),
            prefix: new HarmonyMethod(GetType(), nameof(ShouldTimePassForMachine_Prefix))
        );
        harmony.Patch(
            AccessTools.Method(typeof(Object), nameof(Object.ShouldWobble)),
            prefix: new HarmonyMethod(GetType(), nameof(ShouldWobble_Prefix))
        );
        harmony.Patch(
    ...
    private static void Draw_Postfix(Object __instance, SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
    {
        ...
        if (ModEntry.PoleManager.IsTilePowered(__instance.Location, __instance.TileLocation))
            return;
        float num1 = (float) ((y + 1) * 64 / 10000.0 + __instance.TileLocation.X / 50000.0);
        ...
        spriteBatch.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 - 8, y * 64 - 96 - 16 + num2)), new Rectangle(141, 465, 20, 24), Color.White * 0.75f, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, num1 + 1E-06f);
        spriteBatch.Draw(ModEntry.AssetManager.IconsTexture, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, y * 64 - 64 - 8 + num2)), ModEntry.AssetManager.IconsTextureBoundsMap[IconsTextureItem.NoPowerIcon], Color.White * 0.75f, 0.0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, num1 + 1E-05f);
    }
}