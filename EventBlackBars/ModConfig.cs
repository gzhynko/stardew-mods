...
public class ModConfig
{
    ...
    public static void SetUpModConfigMenu(ModConfig config, ModEntry mod)
    {
        ...
        api.AddNumberOption(manifest, () => (float)config.BarHeightPercentage, val => config.BarHeightPercentage = val,
            () => "Bar Height Percentage", () => "The percentage of the height of the screen for a bar to take up.", 1f,
            49f, 1f);
        api.AddBoolOption(manifest, () => config.MoveBarsInSmoothly, val => config.MoveBarsInSmoothly = val,
            () => "Move Bars In Smoothly",
            () => "Whether to gradually move the bars in when an event starts, or have them fully out right away.");
    }
}