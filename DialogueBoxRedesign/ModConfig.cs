...
public class ModConfig
{
    ...
    public int BoxBackgroundGradientLead { get; set; } = 80;

    /// <summary>
    /// Whether to change the design of the regular portrait dialogue box.
    /// </summary>
    public bool ChangeDialogueDesign { get; set; } = true;

    /// <summary>
    /// Whether to change the design of the question box.
    /// </summary>
    public bool ChangeQuestionDesign { get; set; } = true;
    ...
    /// <summary>
    /// Whether to draw an outline around dialogue text instead of a shadow to improve contrast.
    /// </summary>
    public bool DrawTextOutline { get; set; } = true;
    ...
    public static void SetUpModConfigMenu(ModConfig config, ModEntry mod)
    {
        ...
        api.AddSectionTitle(manifest, () => "Behavior");

        api.AddBoolOption(manifest, () => config.ChangeDialogueDesign, val => config.ChangeDialogueDesign = val,
            () => "Change Dialogue Design",
            () => "Whether to change the design of the regular portrait dialogue box.");
        api.AddBoolOption(manifest, () => config.ChangeQuestionDesign, val => config.ChangeQuestionDesign = val,
            () => "Change Question Design",
            () => "Whether to change the design of the question box.");
        ...
        api.AddNumberOption(manifest, () => config.BoxBackgroundOpacity, val => config.BoxBackgroundOpacity = val,
            () => "Box Background Opacity", null, 0.0f, 1.0f, 0.05f);
        api.AddNumberOption(manifest, () => config.BoxBackgroundGradientLead,
            val => config.BoxBackgroundGradientLead = (int)val,
            () => "Box Background Gradient Lead",
            () =>
                "The padding at the top of the dialogue box for the gradient backdrop. Higher values raise the top of the gradient, potentially increasing contrast and readability.",
            0, 200, 1);
        api.AddBoolOption(manifest, () => config.DrawTextOutline, val => config.DrawTextOutline = val,
            () => "Draw Text Outline",
            () => "Whether to draw an outline around dialogue text instead of a shadow to improve contrast.");
    }
}